...
<form action="{{ route('examens.destroy', $examen->id) }}" method="post">
    <div class="modal-body">
        @csrf
        @method('DELETE')
        <h5 class="text-center">Weet je het zeker dat je {{ $examen->vak }} {{ $examen->examen }} wilt verwijderen?</h5>
    </div>
    <div class="modal-footer">
        <!-- <button type="button" class="button btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
        <button type="submit" class="btn btn-danger">Ja, verwijder het examen</button> -->  
            <div class="mt-4">
