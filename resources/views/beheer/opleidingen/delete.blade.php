...
<form action="{{ route('opleidingen.destroy', $opleiding->id) }}" method="post">
    <div class="modal-body">
        @csrf
        @method('DELETE')
        <h5 class="text-center">Weet je het zeker dat je de opleiding <span class="fc-secondary">"{{ $opleiding->opleiding }}"</span> met het crebo nummer <span class="fc-secondary">"{{ $opleiding->crebo_nr }}"</span> wilt verwijderen?</h5>
    </div>
