...
<form action="{{ route('moments.destroy', $moment->id) }}" method="post">
    <div class="modal-body">
        @csrf
        @method('DELETE')
        <h5 class="text-center">Weet je het zeker dat je het moment van <span class="fc-secondary">"{{ $moment->tijd }}"</span> op <span class="fc-secondary">"{{ $moment->datum }}"</span> wilt verwijderen?</h5>
    </div>
